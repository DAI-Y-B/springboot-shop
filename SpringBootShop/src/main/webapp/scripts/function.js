...
/*function showChater()
{
	...
	_a.setAttribute("href", window.location.href);
	_a.onclick = openRoom;
...
function inArray(array, str)
{
	for(a in array) {
		if(array[a] == str){
//			var number = document.getElementById("number_id_"+ str);
//			number.value++;
			return true;
		}
	}
...
function countTotalMoney(id){
	var price = document.getElementById("price_id_" + id).getElementsByTagName("input")[0].value;
	var number = document.getElementById("number_id_" + id).value;
	return number*price;
}
...
function reloadPrice(id,stock ,status)
{
	var price = document.getElementById("price_id_" + id).getElementsByTagName("input")[0].value;
